...
def train(x, y, vocab, input_dim, hidden_dim):
    model = lstm_lm(hidden_dim, len(vocab), len(vocab), embedding_dim=input_dim).cuda()
...
def main():
    ...
    for dim in [(16,256),(32,265),(256,256)]:
        input_dim = dim[0]
        hidden_dim = dim[1]
        print("dimenison (inp, hidden)")
        print(dim)
        model, loss_func = train(trn_X, trn_Y, trn_vocab, input_dim, hidden_dim)
        # save_model(model)
